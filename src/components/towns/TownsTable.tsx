...
export default function TownsTable() {
  ...
  const [searchResults, setSearchResults] = useState<typeof towns>([]);
  const [searchPages, setSearchPages] = useState(1);

  const { towns, loading, totalPages, refetch } = useTowns(page, limit);
  ...
  const filteredTowns = useMemo(() => {
    const trimmed = searchTerm.trim();
    if (!trimmed) return towns;
    return searchResults;
  }, [towns, searchResults, searchTerm]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        {/* Add Town Button */}
        <AddTownButton onSuccess={refetch} />
      </div>
